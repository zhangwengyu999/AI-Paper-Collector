...
'''
nohup python -u crawler_by_name.py > crawler_by_name.log 2>&1 &
tail -f crawler_by_name.log
'''
...
def search_from_iclr(url, name, res):
    ...
    if name not in res:
        res[name] = []
    for item in tqdm(data["notes"], desc=f"[+ +] Crawling {name}", dynamic_ncols=True):
        res[name].append(
            {
...
def search_from_nips(url, name, res):
    ...
    for paper_item in tqdm(soup.find(class_="col").ul.find_all("li"), desc=f"[+ +] Crawling {name}", dynamic_ncols=True):
...
def search_from_acl(url, tag, name, res):
    ...
    if name not in res:
        res[name] = []
    for tp in tqdm(soup.find_all("p", class_="d-sm-flex align-items-stretch"), desc=f"[+ +] Crawling {name}", dynamic_ncols=True):
        cls = tp.find("strong")
...
def search_from_dblp(url, name, res):
    ...
    for paper_item in tqdm(soup.find_all("li", class_="entry"), desc=f"[+ +] Crawling {name}", dynamic_ncols=True):
        try:
...
def search_from_thecvf(url, name, res):
    ...
    for paper_item in tqdm(soup.find_all("dt", class_="ptitle"), desc=f"[+ +] Crawling {name}", dynamic_ncols=True):
...
def add_code_links(res):
    ...
    r = requests.get(url, headers=HEADERS)
    soup = BeautifulSoup(r.text, "html.parser")
    urls = ['https://github.com'+url['href'] for url in soup.find('table').find_all('a')]
    urls = {url.split('/')[-1][:-3].upper().replace('-','').replace('EUR',''):
            url.replace('github.com', 'raw.githubusercontent.com').replace('blob/','').replace('tree/','') for url in urls}
    for conf in urls:
        code_url = urls[conf]
...
# save the json file separately
def save_res_key_to_file(res, cache_folder, key):
    if key in res:
        with open(f"{cache_folder}/{key}.json", "w") as f:
            json.dump(res[key], f, indent=4)

def crawl(cache_file=None, force=False, cache_folder=None):
    res = {}
    total_count = 0
    ...
    for conf in tqdm(acl_conf, desc="[+] Crawling ACL", dynamic_ncols=True):
        ...
        total_count += len(res[name])
        # res = add_code_links(res)
        save_res_key_to_file(res, cache_folder, name)
        res = {}
...
def do_crawl(cache_file=None, force=False, cache_folder=None):
    if force or cache_file is None or not os.path.exists(cache_file):
        print(f"[+] Crawling papers...")
        res = crawl(cache_file, force, cache_folder)
...
if __name__ == "__main__":
    do_crawl(cache_file="cache/cache.json", force=True, cache_folder="./cache/")
